...
import { 
  ...
  Layers, Trophy, Calendar, Clock, Bike, AlertTriangle, Sparkles, Navigation, CheckCircle, RefreshCw
} from 'lucide-react';
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
            ...
            <div className="bg-slate-50/20 border border-slate-200/60 p-5 rounded-2xl space-y-4">
              ...
              {track.surfaceStats && track.surfaceStats.length > 0 ? (
                ...
                <div className="bg-blue-50/50 border border-dashed border-blue-150 p-4 rounded-xl text-center flex flex-col items-center justify-center space-y-2 min-h-[140px]">
                  <AlertTriangle className="w-6 h-6 text-blue-600 mb-0.5" />
                  <h5 className="font-bold text-slate-700 text-xs">Keine Oberflächen-Daten vorhanden</h5>
                  <p className="text-[10px] text-slate-500 max-w-xs leading-normal">
                    Starte die OpenStreetMap-Analyse, um Straßen, Schotter- und Waldwege dieser Strecke aufzuschlüsseln.
                  </p>
                  {onAnalyzeSurface && (
                    <button
                      type="button"
                      disabled={isAnalyzing}
                      onClick={() => onAnalyzeSurface(track.id, true)}
                      className={`mt-1 inline-flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold rounded-lg text-white shadow-sm transition-all cursor-pointer ${
                        isAnalyzing
                    ...
                    >
                      ...
                      {isAnalyzing ? "Analyse läuft..." : "Jetzt per OSM analysieren"}
                    </button>
                  )}
                </div>
              )}
                    ...
                    {trackClimbs.map((climb, index) => (
                      <div 
                        ...
                        onClick={() => {
                          ...
                          if (segmentPoints.length > 0 && onSelection) {
                        ...
                        className="bg-white hover:bg-indigo-50/60 dark:hover:bg-indigo-950/40 border border-slate-100 dark:border-slate-800 hover:border-indigo-200 p-2.5 rounded-xl text-xs flex justify-between items-center transition-all cursor-pointer group font-sans shadow-2xs"
                        title="Auf Karte zoomen & Bericht schließen"
                      >
                        <div>
                        ...
                        <div className="text-right font-mono">
                          <span className="font-black text-slate-700 dark:text-slate-300 block text-[11px]">{climb.avgGradient.toFixed(1)}% <span className="text-[9px] text-slate-400 font-normal">Ø</span></span>
